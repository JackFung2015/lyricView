package com.zy.lrcviewdemo.lrc.entity;
 ...
 * @Author: fzy
 * @Date: 2022/11/3
 * @Description:歌词行,包括该行歌词的时间，歌词内容
 */
    ...
    public static List<LrcRow> createRows(String standardLrcLine) {
        ...
        try {
            if (standardLrcLine.indexOf("[") != 0 || standardLrcLine.indexOf("]") != 9) {
                if ((standardLrcLine.indexOf("[ar:") != -1) || (standardLrcLine.indexOf("[ti:") != -1)
                        || (standardLrcLine.indexOf("[by:") != -1)) {
                    String content = standardLrcLine.substring(standardLrcLine.indexOf(":") + 1, standardLrcLine.indexOf("]"));
                    List<LrcRow>list = new ArrayList<>();
                    list.add(new LrcRow("0", 0, content));
                    return list;
                }
                return null;
            }
